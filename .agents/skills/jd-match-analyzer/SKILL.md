...
description: |
  Ingests target Job Descriptions (JDs), parses key requirements into structured snippets and tags, stores them in the vault, and analyzes match coverage, skill gaps, and bar-raiser quality against existing resume bullets.
...
Internalizes Job Descriptions as the "question" that the resume is designed to answer. Operates in four complementary modes: Ingest (JD → Vault), Gap Analysis (Vault vs. JD), Bar-Raiser Gap Analysis, and JD-Sourced Skill Expansion.
...
3. **Anti-Regurgitation Rule**: When drafting new bullets from JD signals, never rephrase text the user already has. Mine the JD for technologies and mechanisms the vault doesn't yet cover and draft entirely new content.
...
1. Parse the JD text:
   ...
   - Classify each extracted signal as either `covered` (exists in vault) or `uncovered` (expansion opportunity).
...
2. **Vocabulary Alignment Check**: For each JD requirement, verify whether the matching vault bullet uses the JD's *specific* architectural vocabulary:
...
1. **Extract Uncovered Signals**: From the parsed JD, identify every technology, architectural pattern, methodology, and scale metric that has zero matching bullets in the vault.
2. **Draft Expansion Bullets**: For each uncovered signal cluster, draft 1–2 new accomplishment bullets that credibly weave the signal into the user's work history. These must:
   ...
   - NOT duplicate or rephrase any existing vault bullet
3. **Categorize by Value**: Group expansion bullets by hiring manager priority:
   - 🔴 **Must-Have Signals**: Technologies the JD lists as "required" with no vault coverage
...
5. **Vault Integration**: Selected expansion bullets are added as inactive alternates:
   ```bash
